...
void do_bgfg(char **argv) 
{
  ...
  pid_t pid;
  if (argv[1]==NULL){
    printf("Command requires PID of %%jobid argument\n");
    return;
  }
  else if (argv[1][0]=='%'){ //if argv has the form bg/fg %jid
    jid = atoi(++argv[1]);
  ...
  else if (atoi(argv[1])){ //if argv has the form bg/fg pid
    pid = atoi(argv[1]);
  ...
  else{//if argv does not have either form bg/fg pid or bg/fg %jid
...
void sigint_handler(int sig) 
{
  ...
  kill(-pid, sig); //negative pid because the signal is sent to the processes that share the same pdig
  return;
}
